...
class Chess:
    ...
    def __randomPassword__(self) -> str:
        return f"{Utils.get_full_name().lower().replace(', ', '')}{random.randint(111111, 999999)}Radu"
    
    def __randomUsername__(self) -> str:
        return f"{Utils.get_full_name().lower().replace(', ', '')}{random.randint(111111, 999999)}"
        ...
        try:
            ...
            for game in r:
                for user in game["players"]:
                    user_uuid = user["uuid"]
                    Logger.success(f"Successfully Scraped UUID,{user_uuid},{username}")
